...
positions.forEach((pos, index) => {
  ...
  console.log(`Position ${index + 1}: (${pos.x.toFixed(1)}, ${pos.y.toFixed(1)}, ${pos.z.toFixed(1)}) → (${snapped.x}, ${snapped.y}, ${snapped.z})`);
});
...
console.log(`Back to World: (${backToWorld.x}, ${backToWorld.y}, ${backToWorld.z})`);
...
[0.5, 1, 2, 5].forEach(cellSize => {
  ...
  console.log(`Cell Size ${cellSize}: (${testPos.x}, ${testPos.y}, ${testPos.z}) → (${snapped.x}, ${snapped.y}, ${snapped.z})`);
});
...
testPositions.forEach((pos, index) => {
  ...
  console.log(`Position ${index + 1}: (${pos.x}, ${pos.y}, ${pos.z}) - ${isOnGrid ? "ON GRID" : "OFF GRID"}`);
});
...
console.log(`Completed ${iterations} grid operations in ${duration.toFixed(2)}ms`);
...
const configurations = [
  { name: "Fine Detail", cellSize: 0.5, description: "High precision building" },
...
configurations.forEach(config => {
  ...
  console.log(`${config.name} (${config.cellSize}): (${samplePos.x}, ${samplePos.y}, ${samplePos.z}) → (${snapped.x}, ${snapped.y}, ${snapped.z}) - ${config.description}`);
});
...
export const GridSystemExample = {
  demonstrateSnapToGrid: (position: Vector3, cellSize: number = 1) => {
    ...
    console.log(`Snapped (${position.x.toFixed(2)}, ${position.y.toFixed(2)}, ${position.z.toFixed(2)}) to (${snapped.x}, ${snapped.y}, ${snapped.z})`);
    return snapped;
  },
  
  validateGridPosition: (position: Vector3, cellSize: number = 1) => {
    const isValid = GridUtils.isOnGrid(position, cellSize);
    console.log(`Position (${position.x}, ${position.y}, ${position.z}) is ${isValid ? "valid" : "invalid"} for grid with cell size ${cellSize}`);
    return isValid;
  },
  ...
  performanceTest: (iterations: number = 1000) => {
    ...
    for (let i = 0; i < iterations; i++) {
      const pos = new Vector3(Math.random() * 10, Math.random() * 10, Math.random() * 10);
      GridUtils.snapToGrid(pos, 1);
    }
    
    const duration = performance.now() - startTime;
    console.log(`${iterations} operations completed in ${duration.toFixed(2)}ms`);
    return duration;
  }
};