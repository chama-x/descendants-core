...
export function exampleBasicBlockCreation() {
  ...
  if (!duplicateResult.success) {
    console.log(
      `✅ Correctly prevented duplicate placement: ${duplicateResult.errors.join(", ")}`,
    );
  }
}
...
export function exampleBlockPatterns() {
  ...
  console.log(
    `🔲 Hollow rectangle pattern created with ${rectanglePositions.length} positions`,
  );
  ...
  console.log(
    `✅ Rectangle: ${rectangleResult.successful} blocks created, ${rectangleResult.failed} failed`,
  );
}
...
export function exampleBlockInformation() {
  ...
  allBlockTypes.forEach((info) => {
    ...
    console.log(
      `    Color: ${info.color}, Can glow: ${info.canGlow}, Texture: ${info.textureUrl || "None"}`,
    );
  });
  ...
  console.log(
    `  By type: Stone=${stats.byType.stone}, Leaf=${stats.byType.leaf}, Wood=${stats.byType.wood}`,
  );
...
export function exampleWorldExportImport() {
  ...
  if (exportResult.success && exportResult.data) {
    ...
    console.log(
      `📅 Exported at: ${new Date(exportResult.data.metadata.exportedAt).toISOString()}`,
    );
    ...
    store.clearWorld();
    console.log(
      `World cleared. Block count: ${useWorldStore.getState().blockCount}`,
    );
    ...
    if (importResult.success) {
      ...
      console.log(
        `📊 Block count after import: ${newBlockCount} (original: ${originalBlockCount})`,
      );
    } else {
