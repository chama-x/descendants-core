...
import { MobileShaderProvider, useMobileShaders } from '../components/optimization/MobileShaderManager'
...
function MobileVoxelWorld() {
  ...
  const placeBlock = useCallback((position: Vector3, type: BlockType) => {
    ...
    const newBlock: VoxelBlock = {
      ...
      color: new Color().setHex(materials[type].color as number),
      id: blockId
    }
  ...
  return (
    <>
      ...
      <div className="absolute top-4 left-4 z-10 bg-black bg-opacity-80 p-4 rounded-lg text-white">
        ...
        <div className="mb-2">
          <div className="text-sm text-gray-300">Device: {deviceProfile.deviceType}</div>
          <div className="text-sm text-gray-300">Performance: {deviceProfile.performanceTier}</div>
          <div className="text-sm text-gray-300">WebGL2: {deviceProfile.supportsWebGL2 ? '✅' : '❌'}</div>
          <div className="text-sm text-gray-300">WebGPU: {supportsWebGPU ? '✅' : '❌'}</div>
        </div>
        ...
        <div className="text-xs text-gray-400">
          <div>Blocks: {blocks.size}/{isLowEnd ? '1K' : isMobile ? '5K' : '10K'}</div>
        ...
        {recommendations.length > 0 && (
          <div className="mt-2 p-2 bg-yellow-900 bg-opacity-50 rounded text-xs">
            <div className="font-semibold text-yellow-300">Recommendations:</div>
        ...
        >
          <MobileShaderProvider performanceTarget={
            ...
            {Object.entries(materials).map(([typeStr, material]) => {
              ...
              const typeBlocks = Array.from(blocks.values()).filter(block => block.type === type)
              ...
              return (
                <MobileInstanceManager
                  ...
                  material={material}
                  config={{
                    maxInstances: Math.min(typeBlocks.length * 2, config.maxInstances),
    ...
    return (
      <div className="flex items-center justify-center h-screen bg-gray-900 text-white">
        <div className="text-center">
          ...
          <p className="mb-4">Your device may not support the required WebGL features.</p>
          <button
