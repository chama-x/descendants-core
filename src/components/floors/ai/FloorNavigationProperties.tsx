...
export class FloorNavigationAnalyzer {
  static analyzeFloorForAI(floor: FrostedGlassFloor): AINavigationProperties {
    ...
    return {
      ...
      soundTransmission: this.calculateSoundTransmission(
        thickness,
        transparency,
      ),
  ...
  private static assessSlipperiness(
    roughness: number,
    transparency: number,
  ): boolean {
  ...
  private static calculateVisibilityFactor(
    transparency: number,
    roughness: number,
  ): number {
  ...
  private static calculateSoundTransmission(
    thickness: number,
    transparency: number,
  ): number {
  ...
  private static assessStructuralConfidence(floor: FrostedGlassFloor): number {
    ...
    confidence += (typeModifiers as any)[glassType] || 0;
  ...
  private static calculateNavigationCost(floor: FrostedGlassFloor): number {
    ...
    const slippery = this.assessSlipperiness(
      floor.roughness,
      floor.transparency,
    );
  ...
  private static calculateAlternativePathWeight(
