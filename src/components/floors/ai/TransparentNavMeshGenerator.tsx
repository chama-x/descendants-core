...
export class TransparentNavMeshGenerator {
  ...
  private determineSpecialProperties(
    floor: FrostedGlassFloor,
    aiProps: AINavigationProperties,
  ): string[] {
    ...
    if (aiProps.structuralConfidence < 0.5)
      properties.push("structural_concern");
    if (aiProps.visibilityFactor > 0.6) properties.push("see_through");
    if (
      aiProps.safetyLevel === "dangerous" ||
      aiProps.safetyLevel === "avoid"
    ) {
  ...
  private generateFloorEdgeNodes(
    floor: FrostedGlassFloor,
    aiProps: AINavigationProperties,
  ): NavMeshNode[] {
    ...
    edgePositions.forEach((pos, index) => {
      const edgeNode: NavMeshNode = {
        ...
        floorId: floor.id,
        specialProperties: [
          ...this.determineSpecialProperties(floor, aiProps),
          "edge_node",
        ],
      };
  ...
  private canConnect(nodeA: NavMeshNode, nodeB: NavMeshNode): boolean {
    ...
    if (
      nodeA.specialProperties.includes("hazardous") ||
  ...
  private createEdge(
    ...
    const specialProps = [
      ...nodeA.specialProperties,
      ...nodeB.specialProperties,
    ];

    if (specialProps.includes("slippery"))
      restrictions.push("careful_movement");
    if (specialProps.includes("requires_caution"))
      restrictions.push("slow_movement");
    if (specialProps.includes("high_transparency"))
      restrictions.push("visual_confirmation");

    // Determine safety requirement for this edge
    const safetyLevels = [nodeA.safetyLevel, nodeB.safetyLevel];
    const highestRiskLevel = safetyLevels.includes("dangerous")
      ? "dangerous"
      : safetyLevels.includes("risky")
        ? "risky"
        : safetyLevels.includes("caution")
          ? "caution"
          : "safe";
  ...
  private addAlternativePathNodes(
    navMesh: NavMesh,
    floors: FrostedGlassFloor[],
  ): void {
    floors.forEach((floor) => {
  ...
  private generateAlternativePathNodes(
    floor: FrostedGlassFloor,
    aiProps: AINavigationProperties,
  ): NavMeshNode[] {
  ...
  private connectAlternativeNodes(
    navMesh: NavMesh,
    altNodes: NavMeshNode[],
  ): void {
    ...
    altNodes.forEach((altNode) => {
      ...
      const nearbyNodes = safeMainNodes.filter(
        (mainNode) => altNode.position.distanceTo(mainNode.position) <= 3.0,
      );

      nearbyNodes.slice(0, 2).forEach((mainNode) => {
        // Connect to up to 2 nearby nodes
        const edge: NavMeshEdge = {
          from: altNode.id,
          to: mainNode.id,
  ...
  private removeRedundantNodes(navMesh: NavMesh): void {
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        ...
        if (
          nodeA.position.distanceTo(nodeB.position) < proximityThreshold &&
          nodeA.safetyLevel === nodeB.safetyLevel &&
          nodeA.cost === nodeB.cost
        ) {
          // Merge nodes - keep the one with more connections
          const nodeToKeep =
            nodeA.connectedNodes.length >= nodeB.connectedNodes.length
              ? nodeA
              : nodeB;
    ...
    nodesToRemove.forEach((nodeId) => {
      ...
      // Update edges
      navMesh.edges = navMesh.edges.filter(
