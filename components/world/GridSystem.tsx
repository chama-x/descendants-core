...
export default function GridSystem({ 
  ...
  useFrame((state) => {
    if (materialRef.current) {
      ...
      if (activeRipple) {
        ...
        materialRef.current.uniforms.rippleCenter.value.copy(activeRipple.center);
        materialRef.current.uniforms.rippleTime.value = elapsed;
        materialRef.current.uniforms.rippleStrength.value = activeRipple.strength;
      } else {
  ...
  const handleMouseMove = useCallback((event: MouseEvent) => {
    ...
    const intersected = raycaster.ray.intersectPlane(gridPlane, intersectionPoint);

    if (intersected) {
      // Snap to grid
      const snappedPosition = new Vector3(
        Math.round(intersectionPoint.x / gridConfig.cellSize) * gridConfig.cellSize,
        0,
        Math.round(intersectionPoint.z / gridConfig.cellSize) * gridConfig.cellSize
      );
