...
import { Block as BlockType, BLOCK_DEFINITIONS, BlockType as BlockTypeEnum } from '../../types/blocks'
...
export const Number4Block: React.FC<Number4BlockProps> = ({
  ...
  useFrame((state) => {
    ...
    if (number4GroupRef.current && animated) {
      number4GroupRef.current.rotation.z = Math.sin(state.clock.elapsedTime * 1.5) * 0.1
    }

    // Animate glow effect
    if (glowRef.current) {
      const intensity = 0.5 + Math.sin(state.clock.elapsedTime * 4) * 0.3
      ;(glowRef.current.material as THREE.MeshBasicMaterial).opacity = intensity * 0.2 * glowIntensity
    }
  ...
  const blockMaterial = useMemo(() => {
    return new THREE.MeshStandardMaterial({
      ...
      emissiveIntensity: (blockDef.emissiveIntensity || 0.3) * (block.metadata.glow || 1) * glowIntensity,
    })
  ...
  const createNumber4Geometry = () => {
    ...
    leftVertical.position.set(-0.15, 0.1, 0)
    ...
    rightVertical.position.set(0.15, -0.08, 0)
