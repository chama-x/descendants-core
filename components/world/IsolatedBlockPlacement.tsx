...
export function IsolatedBlockPlacement() {
  ...
  const { queueUpdate, deferHeavyOperation, canExecuteHeavy } =
    useBatchedUpdates("block-placement");
  ...
  const handleClick = useCallback(
    (event: MouseEvent) => {
      if (selectionMode !== SelectionMode.PLACE || placementState.isPlacing)
        return;
      ...
      if (raycaster.ray.intersectPlane(groundPlane, intersectionPoint)) {
        ...
        if (!hasExistingBlock && !atLimit) {
          ...
          const placementOperation = () => {
            ...
            const averageTime =
              placementTimesRef.current.reduce((sum, time) => sum + time, 0) /
              placementTimesRef.current.length;
  ...
  const handleRightClick = useCallback(
    (event: MouseEvent) => {
      ...
      if (intersects.length > 0) {
        ...
        if (blockId) {
          deferHeavyOperation(() => {
            // Get the block ID from the intersected object
            const blockId = intersects[0].object.userData?.blockId;
            if (blockId) {
              // Find the block by ID to get its exact position
              const block = blockMap.get(
                Object.keys(blockMap).find(
                  (key) => blockMap.get(key)?.id === blockId,
                ) || "",
              );
              if (block) {
  ...
  const GhostPreview = React.memo(() => {
    if (!placementState.ghostPosition || selectionMode !== SelectionMode.PLACE)
      return null;

    return (
      <mesh position={placementState.ghostPosition} visible={true}>
