...
import { Block as BlockType, BlockType as BlockTypeEnum, BLOCK_DEFINITIONS } from '../../types/blocks'
...
export const Block: React.FC<BlockProps> = ({
  ...
  useFrame((state) => {
    if (meshRef.current && animated) {
      ...
      meshRef.current.position.y = block.position.y + Math.sin(state.clock.elapsedTime * 2) * 0.1
    }
  })
  ...
  const material = useMemo(() => {
    ...
    if (blockDef.emissive && block.metadata.glow) {
      ...
      materialProps.emissiveIntensity = (blockDef.emissiveIntensity || 0.1) * block.metadata.glow
    }
  ...
  return (
    ...
    >
      ...
      {block.type === BlockTypeEnum.FROSTED_GLASS && (
        <FrostedGlassEffects />
      )}
    </group>
  )
...
const Number4Content: React.FC<{ position: [number, number, number] }> = ({ position }) => {
