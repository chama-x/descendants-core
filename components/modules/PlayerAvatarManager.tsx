...
import { Vector3, Euler, AnimationMixer, AnimationAction, Object3D, Clock } from 'three'
...
export function PlayerAvatarManager({
  ...
  // Avatar loader hook
  const { loadAvatar, isLoading, getMemoryUsage, clearCache } = usePlayerAvatarLoader({
    onAvatarLoaded,
    onError,
    performanceMode
  })
  ...
  const [currentMovementState, setCurrentMovementState] = useState<MovementAnimationState>('idle')
  const [animations, setAnimations] = useState<Map<string, ManagedAnimation>>(new Map())
  ...
  const avatarManagerAPI: IPlayerAvatarManager = useMemo(() => ({
    ...
    unloadAvatar(): void {
      if (playerAvatar && avatarObjectRef.current) {
        ...
        console.log('🗑️ Player avatar unloaded')
      }
    },
    ...
    updateAvatarTransform(position: Vector3, rotation: Euler): void {
      if (playerAvatar && avatarObjectRef.current) {
        ...
        if (deltaTime > 0) {
          velocityRef.current.copy(position).sub(lastPositionRef.current).divideScalar(deltaTime)
    ...
    transitionToAnimation(name: string, duration: number = 0.3): void {
      ...
      const fromAction = animations.get(playerAvatar.currentAnimation)?.action
      const toAction = playerAvatar.currentAnimations.get(name)

      if (!toAction) {
        console.warn(`Target animation '${name}' not found`)
        return
      }
    ...
    blendAnimations(primary: string, secondary: string, weight: number): void {
      ...
      if (!primaryAction || !secondaryAction) {
        console.warn('One or both blend animations not found')
        return
      }
    ...
    saveAvatarState(): void {
      if (playerAvatar) {
        localStorage.setItem('player-avatar-state', JSON.stringify({
  ...
  }), [playerAvatar, animations, scene, camera, isLoading, enableAnimations, onAnimationChanged, onError])

  // Avatar update loop
  const avatarUpdateLoop = useCallback((deltaTime: number) => {
  ...
  }, [playerAvatar, isInitialized, animations, enableAnimations, hideInFirstPerson, activeCamera])
  ...
  useEffect(() => {
    if (!isInitialized && !isLoading) {
      avatarManagerAPI.loadAvatar(modelUrl).catch(console.error)
    }
  ...
  return (
    <group name="player-avatar-manager">
      {debugInfo}
    </group>
  )
}
