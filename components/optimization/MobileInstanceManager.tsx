...
class MobileInstancedRenderer {
  ...
  private createInstancedMesh(geometry: BufferGeometry, material: Material): void {
  ...
  private optimizeGeometry(geometry: BufferGeometry): BufferGeometry {
    ...
    // Optimize indices
    if (!optimized.index) {
      optimized.setIndex(Array.from({ length: optimized.attributes.position.count }, (_, i) => i))
    }
  ...
  private setupInstancedAttributes(): void {
    ...
    const matrixAttribute = new InstancedBufferAttribute(
      new Float32Array(this.config.maxInstances * 16), 16
    )
    ...
    const colorAttribute = new InstancedBufferAttribute(
      new Float32Array(this.config.maxInstances * 3), 3
    )
    ...
    const distanceAttribute = new InstancedBufferAttribute(
      new Float32Array(this.config.maxInstances), 1
    )
  ...
  addInstance(id: string, position: Vector3, rotation?: Quaternion, scale?: Vector3, color?: Color): void {
  ...
  updateInstance(id: string, position?: Vector3, rotation?: Quaternion, scale?: Vector3, color?: Color): void {
    const instance = this.instances.get(id)
    if (!instance) return
  ...
  private updateInstanceMatrix(instance: MobileInstanceData): void {
    ...
    const geometry = this.instancedMesh.geometry as InstancedBufferGeometry
    const distanceAttr = geometry.attributes.instanceDistance as InstancedBufferAttribute
  ...
  private cullInstance(instance: MobileInstanceData, cameraPosition: Vector3): void {
    let visible = true
    ...
    // Distance culling
    if (this.config.enableDistanceCulling && instance.distance > this.config.cullingDistance) {
      visible = false
    ...
    if (visible && this.config.enableFrustumCulling) {
      if (!this.frustum.containsPoint(instance.position)) {
        ...
        const nearbyInstances = this.spatialGrid.getInstancesInRadius(instance.position, 2)
        const hasVisibleNearby = nearbyInstances.some(nearby =>
          nearby.id !== instance.id && this.frustum.containsPoint(nearby.position)
        )
  ...
  adjustQualityForPerformance(avgFrameTime: number): void {
    ...
    if (avgFrameTime > targetFrameTime * 1.5) {
      // Reduce quality
      this.config.cullingDistance = Math.max(50, this.config.cullingDistance * 0.9)
      ...
      this.config.cullingDistance = Math.min(200, this.config.cullingDistance * 1.05)
...
export function MobileInstanceManager({
  ...
  useEffect(() => {
    ...
    rendererRef.current = new MobileInstancedRenderer(geometry, material, finalConfig)
  ...
  useFrame((state, deltaTime) => {
    ...
    if (Math.random() < 0.02) { // ~2% chance per frame at 60fps = ~5s
