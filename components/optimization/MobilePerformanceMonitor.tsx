...
class ThermalManager {
  ...
  constructor(
    private onStateChange: (
      state: MobilePerformanceMetrics["thermalState"],
    ) => void,
  ) {}
  ...
  updatePerformance(frameTime: number) {
    ...
    if (this.frameTimings.length >= 60) {
      const recentAvg =
        this.frameTimings.slice(-30).reduce((a, b) => a + b, 0) / 30;
      const overallAvg =
...
function detectDeviceCapabilities(): DeviceCapabilities {
  ...
  if (gl) {
    ...
    capabilities.isLowEnd = lowEndIndicators.some(
      (indicator) =>
...
export function MobilePerformanceMonitor({
  ...
  const calculateGrade = useCallback(
    (fps: number, frameTime: number): PerformanceGrade => {
  ...
  const generateRecommendations = useCallback(
    (currentMetrics: MobilePerformanceMetrics): PerformanceRecommendation[] => {
  ...
  useFrame((state, deltaTime) => {
    ...
    if (
      ...
      frameTimesRef.current.length > 10
    ) {
      const avgFrameTime =
      ...
      const currentMetrics: MobilePerformanceMetrics = {
        ...
        batteryCharging:
          batteryOptimizerRef.current?.getBatteryInfo()?.charging,
    ...
    <div className="fixed top-4 right-4 z-50 bg-black bg-opacity-90 text-white p-4 rounded-lg text-xs font-mono max-w-sm">
      <div className="mb-2">
        ...
        <div className="text-gray-300">
          Grade: {calculateGrade(metrics.fps, metrics.frameTime)}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        <div>
          ...
          <span
            className={
              metrics.fps >= targetFPS ? "text-green-400" : "text-red-400"
            }
          >
...
export function useMobilePerformanceMetrics() {
  ...
  const handleRecommendation = useCallback(
    (newRecommendations: PerformanceRecommendation[]) => {
      setRecommendations(newRecommendations);
    },
    [],
  );
