...
import { useSkyboxStore, useSkyboxCurrentPreset, useSkyboxIsTransitioning } from '../../store/skyboxStore'
import { SkyboxManagerProps, SkyboxError, SkyboxErrorType } from '../../types/skybox'
...
export function SkyboxManager({
  ...
  useEffect(() => {
    if (!isInitializedRef.current) {
      ...
      console.debug('SkyboxManager initialized with config:', {
        transitionDuration,
        enableAtmosphericEffects,
        performanceMode
      })
    }
  }, [transitionDuration, enableAtmosphericEffects, performanceMode, updateConfig, setPerformanceMode])
  ...
  const handleSkyboxChange = useCallback(async (presetId: string) => {
    try {
      clearError()
      ...
      try {
        ...
        skyboxPerformanceMonitor.recordLoadTime(presetId, loadTime, preset.performance.quality)
        ...
        console.debug(`Skybox transition completed: ${presetId} in ${loadTime}ms`)

      } catch (transitionError) {
        const error = transitionError instanceof Error
          ? transitionError
        ...
        if (fallbackTexture && !currentTextureRef.current) {
          currentTextureRef.current = fallbackTexture
          console.debug('Using fallback texture due to load failure')
        }
      }

    } catch (error) {
      const skyboxError = error instanceof SkyboxError
        ? error
        : new SkyboxError(
  ...
  useEffect(() => {
    return () => {
      ...
      if (isTransitioning) {
        console.debug('Cancelling skybox transition on unmount')
      }
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development') {
      console.debug('SkyboxManager render state:', {
        currentSkybox,
        storeCurrentPreset,
        isTransitioning,
  ...
  try {
    ...
    handleRenderError(error instanceof Error ? error : new Error('Render failed'))
    return null
  }
}
