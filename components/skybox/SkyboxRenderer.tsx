...
export function SkyboxRenderer({
  ...
  useEffect(() => {
    if (texture) {
      ...
      if ("backgroundIntensity" in scene) {
        (
          scene as unknown as { backgroundIntensity: number }
      ...
      if ("backgroundBlurriness" in scene) {
        (
          scene as unknown as { backgroundBlurriness: number }
      ...
      devLog("Skybox texture applied to scene", {
        intensity,
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === "development") {
      devLog("SkyboxRenderer updated:", debugInfo);
    }
  }, [debugInfo]);
