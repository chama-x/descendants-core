...
export default function AnimationTestControls({
  ...
  const [showPerformanceDebug, setShowPerformanceDebug] = useState(showPerformancePanel);
  ...
  const simulantArray = useMemo(() => Array.from(simulants.values()), [simulants]);
  ...
  // Animation state tracking
  const [animationStates, setAnimationStates] = useState<Map<string, AnimationStateInfo>>(new Map());
  ...
  const currentAnimationState = useMemo(() => {
    if (!selectedSimulant) return null;
    return animationStates.get(selectedSimulant) || {
  ...
  useEffect(() => {
    ...
    const mapActionToAnimation = (action: string): string => {
      ...
      if (lowerAction.includes('jump') || lowerAction.includes('leap')) return 'jumping';
      if (lowerAction.includes('run') || lowerAction.includes('sprint')) return 'running';
      if (lowerAction.includes('walk') || lowerAction.includes('move')) return 'walking';
      if (lowerAction.includes('build') || lowerAction.includes('construct')) return 'building';
      if (lowerAction.includes('talk') || lowerAction.includes('communicate')) return 'communicating';
      if (lowerAction.includes('think') || lowerAction.includes('analyze')) return 'thinking';
      if (lowerAction.includes('celebrate') || lowerAction.includes('dance')) return 'celebrating';
   ...
   */
  const handleAnimationChange = useCallback((animationKey: AnimationState) => {
    if (!selectedSimulant) {
    ...
    try {
      ...
      const animationData = AVAILABLE_ANIMATIONS.find(anim => anim.key === animationKey);
      if (!animationData) {
      ...
      console.log(`🎬 Animation changed: ${animationKey} for simulant ${selectedSimulant}`);
    } catch (error) {
          ...
          <CardContent className="pt-0 space-y-4">
              ...
              <div className="relative">
                ...
                >
                  {simulantList.map(simulant => (
                    <option key={simulant.id} value={simulant.id} className="bg-black text-white">
                ...
                <div className="bg-black/30 rounded-md p-3 space-y-2">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-sm font-medium ${
                      currentAnimationState.isPlaying ? 'text-green-300' : 'text-gray-300'
                    }`}>
                      {currentAnimationState.isPlaying ? 'Playing' : 'Stopped'}
                    </span>
                  </div>
                  {currentAnimationState.transitionProgress > 0 && currentAnimationState.transitionProgress < 1 && (
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="text-sm">Transition:</span>
                        <span className="text-sm text-yellow-300">
                          {Math.round(currentAnimationState.transitionProgress * 100)}%
                        </span>
                      </div>
                      <div className="w-full bg-white/10 rounded-full h-1">
                        <div 
                          className="bg-yellow-400 h-1 rounded-full transition-all duration-100"
                          style={{ width: `${currentAnimationState.transitionProgress * 100}%` }}
                        />
                      </div>
                    </div>
                  )}
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {AVAILABLE_ANIMATIONS.map((animation) => {
                  const isActive = currentAnimationState?.currentAnimation === animation.key;
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={!selectedSimulant || !currentAnimationState?.isPlaying}
            ...
            {showAdvancedControls && (
              <>
                ...
                <div className="space-y-4">
                  <Label className="text-sm text-white/80">Advanced Settings</Label>
                  
                  {/* Cross-fade Duration */}
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <Label className="text-xs text-white/60">Cross-fade Duration</Label>
                      <span className="text-xs text-white/60">{crossFadeDuration.toFixed(1)}s</span>
                    </div>
                    <Slider
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-white/60">{animationSpeed.toFixed(1)}x</span>
                    </div>
                    <Slider
                  ...
                  <div className="flex items-center justify-between">
                    <Label className="text-xs text-white/60">Real-time Updates</Label>
                    <Switch
            ...
            <div className="text-xs text-white/40 pt-2 border-t border-white/10 space-y-1">
              ...
              <div>• Select different simulants to control multiple characters</div>
...
export type { AnimationTestControlsProps };