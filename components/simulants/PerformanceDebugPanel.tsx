...
export default function PerformanceDebugPanel({
  ...
  return (
    <Card className={`fixed top-4 right-4 bg-black/90 text-white border-gray-700 ${className}`}>
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <div className={`text-xl font-mono ${getPerformanceColor(state.frameRate)}`}>
              {formatNumber(state.frameRate)}
            </div>
          </div>
          <div>
            <div className="text-gray-400">Quality</div>
            <div className={`text-xl font-semibold ${getQualityColor(state.currentQuality.name)}`}>
              {state.currentQuality.name.toUpperCase()}
            </div>
          </div>
          <div>
            <div className="text-gray-400">Memory</div>
            <div className={`text-lg font-mono ${state.isMemoryPressureHigh ? 'text-red-400' : 'text-green-400'}`}>
          ...
          <div>
            ...
            <div className="text-lg font-mono text-blue-400">
              {state.visibleSimulants}/{state.visibleSimulants + state.culledSimulants}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-4 text-sm">
          <div className={`flex items-center space-x-1 ${state.isPerformanceDegraded ? 'text-red-400' : 'text-green-400'}`}>
            <div className={`w-2 h-2 rounded-full ${state.isPerformanceDegraded ? 'bg-red-400' : 'bg-green-400'}`} />
            <span>{state.isPerformanceDegraded ? 'Degraded' : 'Good'}</span>
          </div>
          <div className={`flex items-center space-x-1 ${state.isMemoryPressureHigh ? 'text-yellow-400' : 'text-green-400'}`}>
            <div className={`w-2 h-2 rounded-full ${state.isMemoryPressureHigh ? 'bg-yellow-400' : 'bg-green-400'}`} />
            <span>Memory</span>
          </div>
        </div>
        ...
        {isExpanded && (
          <>
            ...
            <div className="space-y-3">
              <h4 className="text-sm font-semibold text-gray-300">Quality Control</h4>
              <div className="flex space-x-2">
                {(['high', 'medium', 'low'] as const).map((quality) => (
                  <Button
                    key={quality}
                    variant={state.currentQuality.name === quality ? "default" : "outline"}
                    size="sm"
            ...
            {/* Detailed Stats */}
            <div className="space-y-3">
              <h4 className="text-sm font-semibold text-gray-300">Detailed Metrics</h4>
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                <div>
                  ...
                  <div className="font-mono">{state.currentQuality.cullingDistance}u</div>
                </div>
                <div>
                  <div className="text-gray-400">Update Rate</div>
                  <div className="font-mono">{state.currentQuality.animationUpdateRate}Hz</div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <h4 className="text-sm font-semibold text-gray-300">Performance Report</h4>
              <Button
                variant="outline"
                size="sm"
                onClick={() => {
