...
export class MovementAnimationController
  implements IMovementAnimationController
{
  // Animation Actions
  ...
  private animationStates = new Map<
    MovementAnimationState,
  ...
  constructor({
    ...
    this.idle = this.getAnimationAction(animations, [
      "idle",
      "Idle",
      "idle_pose",
    ]);
    this.walk = this.getAnimationAction(animations, [
      "walk",
      "Walk",
      "walking",
    ]);
    this.run = this.getAnimationAction(animations, ["run", "Run", "running"]);
    this.jump = this.getAnimationAction(animations, [
      "jump",
      "Jump",
      "jumping",
    ]);
    this.land = this.getAnimationAction(animations, [
      "land",
      "Land",
      "landing",
    ]);
  ...
  transitionToState(newState: MovementAnimationState, duration?: number): void {
    ...
    if (!fromConfig || !toConfig) {
      devWarn(
        `Invalid animation state transition: ${this.previousState} -> ${newState}`,
      );
      return;
    }
    ...
    devLog(
      `🎭 Animation transition: ${this.previousState} -> ${newState} (${transitionDuration}s)`,
    );
  }
  ...
  private getAnimationAction(
    ...
    devWarn(`No animation found for names: ${possibleNames.join(", ")}`);
    // Return first available animation as fallback
    return (
      animations.values().next().value ||
      new AnimationAction(this.mixer, null, null)
    );
  }
  ...
  private initializeController(): void {
    ...
    devLog("🎮 Movement Animation Controller initialized");
  }
  ...
  private updateAnimationSpeeds(movementSpeed: number): void {
    const speedMultiplier = this.calculateAnimationSpeed(
    ...
    this.animationStates.forEach((config) => {
      if (config.isActive) {
        const finalSpeed =
          speedMultiplier * config.speedScale * this.speedMultiplier;
  ...
  dispose(): void {
    ...
    devLog("🗑️ Movement Animation Controller disposed");
  }
}
...
export function useMovementAnimationController(
  ...
  useEffect(() => {
    if (mixer && animations.size > 0) {
      ...
      devLog("✅ Movement Animation Controller created");
    ...
    mixer,
    animations,
    options.onAnimationChanged,
    options.blendDuration,
    options.speedMultiplier,
  ]);

  const transitionToState = useCallback(
    (state: MovementAnimationState, duration?: number) => {
      controllerRef.current?.transitionToState(state, duration);
    },
    [],
  );

  const updateFromMovement = useCallback(
    (movementState: MovementState, velocity: Vector3) => {
