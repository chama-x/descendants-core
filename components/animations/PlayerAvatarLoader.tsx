...
export class PlayerAvatarLoader implements IPlayerAvatarLoader {
  ...
  async loadPlayerAvatar(modelUrl: string): Promise<GLTF> {
    try {
      ...
      if (!this.validatePlayerModel(gltf)) {
        throw new Error(`Model ${modelUrl} is not compatible with player avatar system`)
      }
      ...
      console.log(`✅ Player avatar loaded: ${modelUrl}`)
      return gltf

    } catch (error) {
      console.error(`❌ Failed to load player avatar: ${modelUrl}`, error)
      throw error
    }
  }
  ...
  validatePlayerModel(gltf: GLTF): boolean {
    ...
    for (const boneName of requiredBones) {
      if (!bones.has(boneName)) {
        console.warn(`Missing required bone: ${boneName}`)
        return false
      }
    }
    ...
    if (triangleCount > maxTriangles) {
      console.warn(`Model too complex: ${triangleCount} triangles (max: ${maxTriangles})`)
      return false
    }
  ...
  private setupIKConstraints(bones: Map<string, Bone>): void {
    ...
    const leftArm = this.createIKChain(['LeftShoulder', 'LeftElbow', 'LeftHand'], bones)
    const rightArm = this.createIKChain(['RightShoulder', 'RightElbow', 'RightHand'], bones)
    ...
    if (rightArm.length > 0) bones.get('RightHand')!.userData.ikChain = rightArm
  ...
  private async cleanupOldModels(): Promise<void> {
    ...
    console.log(`🧹 Cleaned up ${modelsToRemove.length} old avatar models`)
  }
...
export function usePlayerAvatarLoader({
  ...
  if (!loaderRef.current) {
    loaderRef.current = new PlayerAvatarLoader(scene, memoryLimit, performanceMode)
  }

  const loadAvatar = useCallback(async (modelUrl: string): Promise<PlayerAvatarState | null> => {
