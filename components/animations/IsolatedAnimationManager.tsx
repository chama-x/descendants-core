...
import { useAnimationRender, useBatchedUpdates } from '../../hooks/performance/useIsolatedRender'
...
export function IsolatedAnimationManager() {
  ...
  const { queueUpdate, deferHeavyOperation, canExecuteHeavy } = useBatchedUpdates('animation')
  ...
  const updateAnimationsWithBudget = useCallback((deltaTime: number) => {
  ...
  const updateAnimationBlending = useCallback((animation: ManagedAnimation, deltaTime: number) => {
    ...
    if (Math.abs(weightDiff) > 0.01) {
      ...
      animation.currentWeight = Math.max(0, Math.min(1, animation.currentWeight))
  ...
  const registerAnimation = useCallback((
    ...
    if (animations.has(simulantId)) {
      console.warn(`Animation already registered for simulant: ${simulantId}`)
      return
    }
    ...
    queueUpdate(() => {
      setMetrics(prev => ({
        ...
        totalActions: Array.from(animations.values()).reduce((sum, anim) => sum + anim.actions.size, 0)
      }))
    })
  ...
  const playAnimation = useCallback((
    ...
    if (!newAction) {
      console.warn(`Animation ${animationName} not found for simulant: ${simulantId}`)
      return
    }
    ...
    if (animation.currentAction && animation.currentAction !== newAction) {
      ...
      setTimeout(() => {
        ...
        queueUpdate(() => {
          setMetrics(prev => ({ ...prev, blendTransitions: prev.blendTransitions + 1 }))
        })
      }, fadeTime * 1000 / 2) // Half the fade time
    } else {
  ...
  // Stop animation
  const stopAnimation = useCallback((simulantId: string, fadeTime: number = 0.3) => {
    const animation = animations.get(simulantId)
  ...
  const unregisterAnimation = useCallback((simulantId: string) => {
    const animation = animations.get(simulantId)
    if (!animation) return
  ...
  const updateMetrics = useCallback((currentFrameTime: number) => {
    ...
    if (averageFrameTime > 5.0 && maxAnimationsPerFrame.current > 2) {
      ...
      console.warn(`🐌 Reducing animation budget to ${maxAnimationsPerFrame.current} per frame`)
    } else if (averageFrameTime < 2.0 && maxAnimationsPerFrame.current < 10) {
      maxAnimationsPerFrame.current += 1
      console.log(`⚡ Increasing animation budget to ${maxAnimationsPerFrame.current} per frame`)
    }
  }, [animations])
  ...
  useEffect(() => {
    ...
    simulantArray.forEach(simulant => {
      ...
      if (!animations.has(simulant.id)) {
        console.log(`Would register animation for simulant: ${simulant.name}`)
      }
    })
  ...
  }), [registerAnimation, playAnimation, stopAnimation, unregisterAnimation, metrics, renderMetrics])
