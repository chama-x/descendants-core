...
F03 Data Structures: Uses ring buffers, weighted scorers, bloom filters, and vector indices for efficient memory operations.  
F04 UI: Memory Viewer queries snapshot + diff endpoints; redaction toggle; displays memory metrics.  
F05/F06 Physics + Collision: Spatial events ingested if flagged (movement boundary crossing, new contact class).  
F08 LLM: Summarization & embedding call surfaces via LLMManager; if throttled → degrade.  
F09 Rate Governor: Pressure modifies retrieval weights & compression ratio.  
F10 Behavior: AgentContext memory slice resolver uses Retrieval API.
...
21-memory-llm-integration-test/  
  PROMPT_MEMORY_LLM_INTEGRATION_TEST.md  
22-memory-rate-governor-integration-test/  
  PROMPT_MEMORY_RATE_GOVERNOR_INTEGRATION_TEST.md  
23-end-to-end-memory-cycle-test/  
  PROMPT_END_TO_END_MEMORY_CYCLE_TEST.md  

---

## 18. SUB-PROMPT TEMPLATE
```
Feature: F07-MEMORY-SYSTEM
Sub-Task: <NAME>
Context:
...
2. Generate exactly 23 sub-prompts (Section 17 index + 3 integration tests).  
3. Each ≤ 220 lines.  
4. Include explicit file paths + exported symbols.  
5. Include negative test scenarios + error handling validation.  
6. Include 3 integration test sub-prompts for cross-feature validation.
7. Produce INDEX.md summarizing artifacts after generation.  
Validation After Generation:
- COUNT(sub-prompts)=23 (20 core + 3 integration)
- All contain "Feature: F07-MEMORY-SYSTEM"
- Each has Objective, Constraints, Validation, Non-goals
- Integration tests validate cross-feature dependencies
- INDEX summarizes file outputs & key exports
- INDEX lists all outputs + trigger phrase

---

## 26. EXECUTION TRIGGER PHRASE
"GENERATE_F07_SUB_PROMPTS_v1"
