...
Version: 1.0  
Feature ID: F08-GEMINI-INTEGRATION
...
EXISTING_FEATURES=F01 (avatar runtime), F02 (Engine skeleton), F03 (data structures), F04 (UI layer), F05 (physics), F06 (collision), F07 (memory system)
THIS FEATURE DEPENDS_ON=Engine request API + entity registration + memory context loading
OUTPUT CORE: LLMAdapter + LLMManager + Dual Agent Activation + Persona/Memory Loading
RATE AWARENESS=Delegates hard governance to Feature 9 (RateGovernor hook), but must expose usage counters now.
...
# Rate Governance (centralized – consumed by F09 via injection only)
GEMINI_RATE_PER_MINUTE=<int>
GEMINI_BURST_CALLS=<int>
GEMINI_REFILL_RATE=<float>
GEMINI_MAX_DAILY_TOKENS=<int>
GEMINI_SOFT_TOKEN_WINDOW=<int ms>
GEMINI_SOFT_TOKEN_BUDGET=<int tokens>

// Injection Contract (for Rate Governor F09):
...
// }
// export function getGeminiRateConfig(): GeminiRateConfig
...
------------------------------------------------------------
"""
Feature: F08-GEMINI-INTEGRATION
Sub-Task: <NAME>
Context:
