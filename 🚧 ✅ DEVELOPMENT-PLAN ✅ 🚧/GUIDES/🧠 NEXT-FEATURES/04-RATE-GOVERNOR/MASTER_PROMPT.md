...
CURRENT_FEATURES=F01 Avatars, F02 Engine Core, F03 Gemini Integration  
...
C5: Minimal overhead: approval path average < 0.05 ms (baseline).  
...
------------------------------------------------------------
GEMINI_RATE_PER_MINUTE=<int> (soft calls/min limit per primary model)  
GEMINI_BURST_CALLS=<int> (bucket capacity)  
GEMINI_REFILL_RATE=<float> (tokens per second for calls bucket)  
GEMINI_MAX_DAILY_TOKENS=<int> (hard absolute token cap)  
GEMINI_SOFT_TOKEN_WINDOW=<int ms> (window for soft token pacing)  
GEMINI_SOFT_TOKEN_BUDGET=<int tokens> (soft window token threshold)  
...
- COUNT=16
- All include "Feature: F04-RATE-GOVERNOR"
