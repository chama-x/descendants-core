```Descendants/🚧 ✅ DEVELOPMENT-PLAN ✅ 🚧/PROMPTS/🧠 NEXT-FEATURES/06-PHYSICS/MASTER_PROMPT.md#L1-320
# MASTER PROMPT: REAL‑TIME PHYSICS & COLLISION INTEGRATION LAYER
Version: 1.0  
Feature ID: F06-PHYSICS  
Authoring Mode: Realtime Systems / Simulation Architecture / Engine Mediation  
Primary Goal: Introduce a modular, deterministic-friendly physics and collision subsystem (baseline: Rapier via @react-three/rapier / Drei helpers; fallback adapters for Cannon/Ammo) fully mediated by the central Engine (F02), emitting structured collision & trigger events, supporting character controllers for AI simulants and player, and integrating with upcoming Behavior Orchestrator (F05) and Rate Governor awareness (F04) without leaking ungoverned world mutation paths.

------------------------------------------------------------
SECTION 1: ULTRA-COMPRESSED CONTEXT SNAPSHOT
------------------------------------------------------------
WORLD: 3D immersive sim with autonomous agents + human player  
CURRENT CORE: Engine authority, LLM agents planned, avatars integrated  
...
physics:collision:start { a, b, contacts, impactVelocity }
physics:collision:end { a, b }
...
F03 (Gemini): Agents consume spatial queries for environment description (raycast / overlap).  
F04 (Rate Governor): Not directly rate-limited, but heavy spatial queries could become cost metrics (future).  
F05 (Behavior): Uses perception diff with physics-based proximity & ground state.  
...
- All include "Feature: F06-PHYSICS"
