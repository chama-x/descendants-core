```Descendants/🚧 ✅ DEVELOPMENT-PLAN ✅ 🚧/PROMPTS/🧠 NEXT-FEATURES/03-ADVANCED-DATA-STRUCTURES/MASTER_PROMPT.md#L1-400
...
Primary Goal: Introduce a unified, high-performance, memory‑efficient, deterministic data structure & algorithm layer powering Engine (F02), UI Telemetry (F04), Physics & Collision (F05–F06), Memory System (F07), LLM Integration (F08), Rate Governor (F09), and Behavior Orchestrator (F10). Provide scalable primitives (queues, indexed stores, spatial structures, diff hashing, compressed logs, approximate similarity) with strict performance contracts and observability hooks.
...
- UI event streams (bounded ring buffers)
...
AC4: TokenBucketMap approve() path median < 0.002 ms across 10k sequential approvals (baseline dev machine).
...
------------------------------------------------------------
SECTION 6: KEY INTERFACES (VERSIONED)
------------------------------------------------------------
// @version 1.0.0 - Initial data structures API
// @consumers F09-RateGovernor, F05-Physics, F07-Memory, F10-Behavior
...
// @version 1.0.0 - Priority queue interface
...
TokenBucketMap approve | < 0.002 ms p50, < 0.008 ms p95 (10k keys)
DynamicAABBTree update (move) | < 0.08 ms avg
VectorIndex linear search (1k vectors, dim 384) | < 3 ms

// Performance Hierarchy Validation:
// TokenBucket (0.002ms) << Physics step budget (0.08ms) << Rate Governor approval (0.05ms)
// Ensures foundational primitives don't bottleneck dependent systems
...
------------------------------------------------------------
SECTION 20: SUB-PROMPT TEMPLATE (STANDARDIZED FORMAT v1.0)
------------------------------------------------------------
"""
Feature: F03-ADVANCED-DATA-STRUCTURES
Sub-Task: <NAME>

Context:
Data structures layer requires <specific component> aligned with Engine mediation & strict performance contracts.

Objective:
<Clear measurable deliverable - single sentence>
