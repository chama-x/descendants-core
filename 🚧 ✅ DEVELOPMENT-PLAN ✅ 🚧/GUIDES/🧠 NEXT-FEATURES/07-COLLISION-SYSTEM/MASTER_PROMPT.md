...
4. Uniform event contract for Engine + Behavior subsystem (F05).  
...
------------------------------------------------------------
COLLIDER_DUPLICATE_ID  
COLLIDER_UNKNOWN_ID  
COLLIDER_LAYER_UNDEFINED  
COLLIDER_SHAPE_INVALID  
COLLISION_MAX_CONTACTS_EXCEEDED  
COLLISION_PAIR_OVERFLOW  
SENSOR_INVALID_SHAPE  
SWEEP_UNSUPPORTED_SHAPE  
LOS_ITERATION_LIMIT_REACHED  
BVH_REBUILD_THRESHOLD_EXCEEDED  
...
F03 Gemini / F05 Behavior: Perception gathers sensor & LOS results for plan context; predictive sweeps aid movement decisions.  
F04 Rate Governor: Expose query cost metrics (optional rate adaptation if collision queries become expensive).  
F06 Physics: Source of authoritative transforms for dynamic bodies; collision system supplements with sensors & predictive queries.  
F01/F05 Avatar/Behavior: Movement controllers can request predictive sweeps to avoid collisions before physics resolution.  
