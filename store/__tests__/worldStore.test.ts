...
describe('WorldStore', () => {
  ...
  describe('Spatial Hash Map Operations', () => {
    ...
    it('should provide O(1) block lookups', () => {
      ...
      // Add a block
      const success = store.addBlock(position, 'stone', 'human');
  ...
  describe('Block Operations', () => {
    it('should add blocks successfully with collision detection', () => {
      ...
      const success1 = store.addBlock(position, 'stone', 'human');
      ...
      const success2 = store.addBlock(position, 'wood', 'human');
    ...
    it('should remove blocks successfully', () => {
      ...
      // Add block
      store.addBlock(position, 'leaf', 'human');
      store = useWorldStore.getState();
      expect(store.blockCount).toBe(1);
    ...
    it('should remove blocks by ID', () => {
      ...
      // Add block
      store.addBlock(position, 'wood', 'human');
      const block = store.getBlock(position);
      expect(block).toBeDefined();
    ...
    it('should enforce 1000-block limit', () => {
      ...
      for (let i = 0; i < 1000; i++) {
        const success = store.addBlock(new Vector3(i, 0, 0), 'stone', 'human');
        expect(success).toBe(true);
      }
      ...
      const success = store.addBlock(new Vector3(1000, 0, 0), 'stone', 'human');
    ...
    it('should provide efficient block counting', () => {
      ...
      // Add multiple blocks
      for (let i = 0; i < 10; i++) {
        store.addBlock(new Vector3(i, 0, 0), 'stone', 'human');
      }
  ...
  describe('Undo/Redo System', () => {
    it('should save snapshots before changes', () => {
      ...
      store.addBlock(new Vector3(0, 0, 0), 'stone', 'human');
    ...
    it('should undo block operations', () => {
      ...
      // Add block
      store.addBlock(position, 'stone', 'human');
    ...
    it('should clear redo history when new changes are made', () => {
      ...
      // Add two blocks
      store.addBlock(new Vector3(0, 0, 0), 'stone', 'human');
      store.addBlock(new Vector3(1, 0, 0), 'wood', 'human');
      ...
      store.addBlock(new Vector3(2, 0, 0), 'leaf', 'human');
  ...
  describe('World Statistics', () => {
    it('should calculate world statistics correctly', () => {
      ...
      store.addBlock(new Vector3(0, 0, 0), 'stone', 'human');
      store.addBlock(new Vector3(1, 0, 0), 'stone', 'human');
      store.addBlock(new Vector3(2, 0, 0), 'leaf', 'human');
      store.addBlock(new Vector3(0, 1, 0), 'wood', 'human');
  ...
  describe('Utility Functions', () => {
    it('should update selected block type', () => {
      ...
      expect(store.selectedBlockType).toBe('stone'); // default
      
      store.setSelectedBlockType('leaf');
      expect(useWorldStore.getState().selectedBlockType).toBe('leaf');
    });
  ...
  describe('Performance Characteristics', () => {
    it('should handle large numbers of blocks efficiently', () => {
      ...
      // Add 100 blocks
      for (let i = 0; i < 100; i++) {
        store.addBlock(new Vector3(i % 10, Math.floor(i / 10), 0), 'stone', 'human');
      }
