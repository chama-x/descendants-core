...
export function useSkybox(options: UseSkyboxOptions = {}): UseSkyboxReturn {
  ...
  const handleError = useCallback((err: SkyboxError | Error | string) => {
    const skyboxError = err instanceof SkyboxError
      ? err
      : new SkyboxError(
  ...
  const changePreset = useCallback(async (
    presetId: string,
    ...
    try {
      ...
      if (transitionConfig) {
        updateConfig({
          transitionDuration: transitionConfig.duration || config.transitionDuration
        })
      }
  ...
  const preloadAdjacentPresets = useCallback(async (currentPresetId: string) => {
  ...
  const preloadPreset = useCallback(async (presetId: string): Promise<void> => {
    try {
      if (!presets[presetId]) {
        throw new SkyboxError(
          SkyboxErrorType.INVALID_PRESET,
  ...
  const addPreset = useCallback((preset: SkyboxPreset) => {
    try {
  ...
  const toggleAtmosphericEffects = useCallback(() => {
    updateConfig({ enableAtmosphericEffects: !config.enableAtmosphericEffects })
  ...
  const isPresetLoaded = useCallback((presetId: string): boolean => {
    return textureCache.has(presetId)
  }, [textureCache])

  const isPresetLoading = useCallback((presetId: string): boolean => {
    return loadStates[presetId]?.isLoading || false
  }, [loadStates])

  const getPresetLoadState = useCallback((presetId: string) => {
    return loadStates[presetId] || {
      isLoading: false,
      progress: 0,
      error: null
    }
  ...
  useEffect(() => {
    if (autoLoad && !isInitializedRef.current && Object.keys(presets).length > 0) {
 ...
 */
export function useSkyboxPresets() {
  const presets = useSkyboxPresets()
  const { addPreset, updatePreset, removePreset, duplicatePreset } = useSkyboxStore()

  const createPreset = useCallback((preset: Omit<SkyboxPreset, 'id' | 'createdAt' | 'updatedAt'>) => {
  ...
  const updateExistingPreset = useCallback((presetId: string, updates: Partial<SkyboxPreset>) => {
    updatePreset(presetId, { ...updates, updatedAt: new Date().toISOString() })
  }, [updatePreset])
...
export function useSkyboxPerformanceMonitor() {
  const performance = useSkyboxPerformance()
  const { updatePerformanceMetrics, clearCache, optimizeCache } = useSkyboxStore()
  ...
  const getHealthStatus = useCallback(() => {
    ...
    if (performance.memoryUsage > 200 || performance.frameImpact < -10) return 'warning'
    return 'healthy'
  }, [performance])
  ...
  const getOptimizationSuggestions = useCallback(() => {
    ...
    if (performance.loadTime > 3000) {
      suggestions.push('Slow loading detected - check network connection or use lower quality presets')
    }

    if (performance.frameImpact < -15) {
      suggestions.push('Significant frame rate impact detected - consider performance mode')
    }

    if (performance.cacheHitRate < 0.5) {
      suggestions.push('Low cache efficiency - consider preloading frequently used presets')
    }
