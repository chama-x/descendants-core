  ...
  try {
    ...
    return {
      ...
      limits: Object.fromEntries(
        Object.entries(adapter.limits).map(([key, value]) => [
          key,
          value as number,
        ]),
      ),
...
class ThermalManager {
  ...
  constructor(
    private onStateChange: (state: MobileDeviceProfile["thermalState"]) => void,
  ) {}
  ...
  updateFrameTime(frameTime: number) {
    ...
    if (now - this.lastThermalCheck > 2000) {
...
class BatteryManager {
  ...
  private performanceMode: "power-saver" | "balanced" | "performance" =
    "balanced";
...
class AdaptiveQualityManager {
  ...
  updateFrameTime(frameTime: number): number {
    ...
    if (this.frameTimings.length >= 10) {
      const avgFrameTime =
        this.frameTimings.slice(-10).reduce((a, b) => a + b) / 10;
      const targetFrameTime = 1000 / this.targetFPS;

      if (avgFrameTime > targetFrameTime * 1.2) {
        this.currentQuality = Math.max(
          this.minQuality,
          this.currentQuality - this.adjustmentSpeed,
        );
      } else if (avgFrameTime < targetFrameTime * 0.8) {
        this.currentQuality = Math.min(
          this.maxQuality,
          this.currentQuality + this.adjustmentSpeed * 0.5,
        );
      }
    }
  ...
  setQuality(quality: number) {
    this.currentQuality = Math.max(
      this.minQuality,
      Math.min(this.maxQuality, quality),
    );
  }
}
...
export function useMobileOptimization(
  overrides: Partial<MobileOptimizationConfig> = {},
) {
  const { gl, camera, scene } = useThree();

  // Device detection and capabilities
  const [deviceProfile] = useState<MobileDeviceProfile>(() =>
    detectMobileDevice(),
  );
  const [webgpuCapabilities, setWebgpuCapabilities] =
    useState<WebGPUCapabilities | null>(null);
  ...
  // Management systems
  const thermalManagerRef = useRef<ThermalManager | undefined>(undefined);
  const batteryManagerRef = useRef<BatteryManager | undefined>(undefined);
  const qualityManagerRef = useRef<AdaptiveQualityManager | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    if (deviceProfile.deviceType === "mobile") {
      ...
      // Enable mobile-optimized extensions via WebGL context
  ...
  useFrame((state, deltaTime) => {
    ...
    const thermalMultiplier =
      thermalManagerRef.current?.getThrottleMultiplier() || 1.0;

    // Apply battery optimization
    const batteryMultiplier =
      batteryManagerRef.current?.getPerformanceMultiplier() || 1.0;
    ...
    if (finalQuality < 1.0) {
      const adjustedPixelRatio =
        config.pixelRatio * Math.max(0.5, finalQuality);
    ...
    if (now - lastMetricsUpdate.current >= 1000) {
      ...
      const newMetrics: MobilePerformanceMetrics = {
        fps,
        frameTime:
          frameTimesRef.current[frameTimesRef.current.length - 1] || 16.67,
        memoryUsage,
  ...
  const updateConfig = useCallback(
    (updates: Partial<MobileOptimizationConfig>) => {
  ...
  return {
    ...
    recommendations: useMemo(() => {
      ...
      if (metrics.thermalState === "critical") {
        recs.push(
          "Device overheating - immediate quality reduction recommended",
        );
      }

      if (
        metrics.batteryLevel &&
        metrics.batteryLevel < config.lowBatteryThreshold
      ) {
      ...
      if (metrics.drawCalls > config.maxDrawCalls) {
        recs.push(
          "Too many draw calls - enable batching or reduce unique materials",
        );
      }
