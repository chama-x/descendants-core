...
export function useSafeCameraMode(config: Partial<SafeCameraModeConfig> = {}) {
  ...
  const changeCameraMode = useCallback((
    ...
    userInteractionContext.current.intentionalModeChange = reason === 'user' || reason === 'keyboard'
    lastModeChange.current = now

    // Perform the mode change
    setCameraMode(newMode)

    console.log(`Camera mode changed: ${activeCamera} → ${newMode} (reason: ${reason})`)
  ...
  const handleDoubleClickFocus = useCallback((event: MouseEvent) => {
    ...
    console.log('Double-click focus triggered (no mode change)')
    return true
  ...
  const getDebugInfo = useCallback(() => {
    ...
    return {
      ...
      canChangeMode: !isTransitioning && (now - lastModeChange.current) >= finalConfig.minModeChangeDelay,
